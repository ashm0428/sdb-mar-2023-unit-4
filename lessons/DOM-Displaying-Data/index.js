const placeholderQuestions = [
  ...
  // ! INcludes not going to work on an array of objects
  // ! You can do an includes on a string

  let nature = placeholderQuestions.filter((cat) => cat.category === "Nature").splice(5, 5);
  let history = placeholderQuestions.filter((cat) => cat.category === "History");
  let animals = placeholderQuestions.filter((cat) => cat.category === "Animals");
  let computers = placeholderQuestions.filter((cat) => cat.category === "Computers");
  let mythology = placeholderQuestions.filter((cat) => cat.category === "Mythology");
  let general = placeholderQuestions.filter((cat) => cat.category === "General");
  let final = placeholderQuestions.filter((cat) => cat.category === "Final");


// Create a function that will populate Nature Section
populateCategory("nature-title", ".nature", nature);
populateCategory("history-title", ".history", history);
populateCategory("animals-title", ".animals", animals);
function populateCategory(elementTitleId, elementQuestionClass, categoryFilteredArray) {
  // Fish for Title
  ...
  questions.forEach((item, index) => {
    ...
    <input type="text" class="answer-${title.split(" ").join("-").toLowerCase()}" style="display:block"/>`;
  });
}
